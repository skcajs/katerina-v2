...
mod tests {
    ...
    fn hit_is_always_the_lowest_nonnegative_intersection() {
        ...
        let i3 = Intersection::new(-3.0, s_key);
        let i4 = Intersection::new(2.0, s_key);
    ...
    fn precomputing_the_state_of_an_intersection() {
        ...
        assert_eq!(comps.object_key, i.object_key);
    ...
    fn precomputing_the_reflection_vector() {
        ...
        let i = Intersection::new(2_f64.sqrt(), s_key);
        let comps = i.prepare_computations(&r, &vec![]);
        assert_eq!(
            comps.reflectv,
            Tuple::vector(0.0, 2_f64.sqrt() / 2.0, 2_f64.sqrt() / 2.0)
    ...
    fn finding_n1_and_n2_at_various_intersections() {
        ...
        let mut b = glass_sphere().with_transform(Matrix::translation(0.0, 0.0, -0.25));
        b.set_material(b.get_material().clone().with_refractive_index(2.0));
        let b_key = objects.add(b);

        let mut c = glass_sphere().with_transform(Matrix::translation(0.0, 0.0, 0.25));
        c.set_material(c.get_material().clone().with_refractive_index(2.5));
        let c_key = objects.add(c);

        let r = Ray::new(Tuple::point(0.0, 0.0, -4.0), Tuple::vector(0.0, 0.0, 1.0));
        let xs = vec![
            Intersection::new(2.0, a_key),
            Intersection::new(2.75, b_key),
            Intersection::new(3.25, c_key),
            Intersection::new(4.75, b_key),
            Intersection::new(5.25, c_key),
            Intersection::new(6.0, a_key),
        ];
    ...
    fn the_under_point_is_offset_below_the_surface() {
        ...
        let i = Intersection::new(5.0, shape_key);
        let xs = vec![i.clone()];
    ...
    fn the_schlick_approximation_under_total_internal_reflection() {
        ...
        let xs = vec![
            Intersection::new(-2_f64.sqrt() / 2.0, shape_key),
            Intersection::new(2_f64.sqrt() / 2.0, shape_key),
        ];
    ...
    fn the_schlick_approximation_with_a_perpendicular_viewing_angle() {
        ...
        let xs = vec![
            Intersection::new(-1.0, shape_key),
            Intersection::new(1.0, shape_key),
        ];
        let comps = xs[1].prepare_computations(&r, &xs);
        let reflectance = comps.schlick;
    ...
    fn the_schlick_approximation_with_small_angle_and_n2_greater_than_n1() {
        ...
        let xs = vec![Intersection::new(1.8589, shape_key)];
