...
impl Cone {
    ...
    pub fn intersect_caps(&self, ray: &Ray, xs: &mut Vec<f64>) {
...
mod tests {
    ...
    fn intersecting_cone_with_ray() {
        ...
        assert_eq!(xs.len(), 2);
        let delta = 1e-4;
        assert!((xs[0] - 8.66025).abs() < delta);
        assert!((xs[1] - 8.66025).abs() < delta);

        let r = Ray::new(Tuple::point(1.0, 1.0, -5.0), Tuple::vector(-0.5, -1.0, 1.0).normalize());
        let xs = cone.local_intersect(&r);
        assert_eq!(xs.len(), 2);
        let delta = 1e-4;
        assert!((xs[0] - 4.55006).abs() < delta);
        assert!((xs[1] - 49.44994).abs() < delta);
    }
