...
impl World {
    ...
    pub fn with_objects(mut self, objects: Vec<ObjectKey>) -> World {
        self.objects = objects;
        self
    }

    pub fn add_objects(&mut self, objects: Vec<ObjectKey>) {
    ...
    pub fn default_world() -> World {
        ...
        let s1 = Object::sphere().with_material(
            Material::new()
        ...
        let s1_key = store.add(s1);
        let s2 = Object::sphere().with_transform(Matrix::scaling(0.5, 0.5, 0.5));
        let s2_key = store.add(s2);
        World {
            objects: vec![s1_key, s2_key],
            lights: vec![light],
        }
    }
    ...
    pub fn intersect(&self, ray: &Ray) -> Vec<Intersection> {
        ...
        for key in &self.objects {
            if let Some(object) = store.get(*key) {
    ...
    pub fn shade_hit(&self, record: &Record, depth: usize) -> Tuple {
        ...
        if let Some(object) = store.get(record.object_key) {
            for light in &self.lights {
                surface = surface
                    + object.get_material().lighting(
                        &object,
                        light,
                        record.over_point,
                        record.eyev,
    ...
    pub fn reflected_color(&self, record: &Record, depth: usize) -> Color {
        ...
        if let Some(object) = store.get(record.object_key) {
            if object.get_material().reflectivity == 0. {
                return Colors::black();
            }
            ...
            return color * object.get_material().reflectivity;
        }
    ...
    pub fn refracted_color(&self, record: &Record, depth: usize) -> Color {
        ...
        if let Some(object) = store.get(record.object_key) {
            if object.get_material().transparency == 0. {
                return Colors::black();
            }
            ...
            return self.color_at(&refract_ray, depth - 1) * object.get_material().transparency;
        }
...
// mod tests {
//     use crate::{
//         matrix::Matrix, pattern::Pattern, ray::Ray, transformation::Transformation, tuple::Tuple,
//     };
...
//             .with_material(Material::new().with_reflectivity(0.5))
...
//         );
//         let i = Intersection::new(2_f64.sqrt(), &world.objects[2]);
//         let comps = i.prepare_computations(&r, &vec![]);
//         let color = world.reflected_color(&comps, 4);
//         let delta = 0.0001;
...
//         world.add_lights(vec![Light::new(
...
//         let world = World::default_world();
//         let shape = &world.objects[0].with_material(
//             Material::new()
...
//         let world = World::default_world();
//         let a = &world.objects[0].with_material(
//             Material::new()
//                 .with_ambient(1.0)
//                 .with_pattern(Pattern::test_pattern()),
//         );

//         let b = &world.objects[1].with_material(
//             Material::new()
//                 .with_transparency(1.0)
//                 .with_refractive_index(1.5),
//         );
